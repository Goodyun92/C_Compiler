...
int sem_declaration(A_ID *id, int addr){
	...
	switch(id->kind){
		...
		case ID_FIELD:
			i=sem_A_TYPE(id->type);
			if(isFunctionType(id->type) || isVoidType(id->type) || isStructOrUnionType(id->type))
...
{
	if(result.type==int_type && int_type || result.type==char_type && t==char_type || result.type==float_type && t==float_type) ;
...
void semantic_error(int i, int ll, char *s)
{
	...
	switch(i) {
		...
		case 38: printf("illegal kind of identifier %s in expression\n",s);
			break;
		case 39: printf("illegal type size in sizeof operaion\n");
			break;
		...
		case 51: printf("illegal expression type in case label\n");
			break;
		case 57: printf("not permitted type casting in expression\n");
			break;
		case 58: printf("not permitted type casting inexpression\n");
			break;
		...
		case 72: printf("default label not within a switch statement \n");
			break;
		case 73: printf("break statement not within a switch statement \n");
			break;
		...
		case 81: printf("integer type expression required in enumerator\n");
			break;
		case 82: printf("illegal array size of type\n");
			break;
